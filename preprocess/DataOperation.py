    ...
    def __init__(self, database, memory_size:int, sample_no:int):
        ...
        for i in range(len(database['sequences'])):
            ...
            for j in range(sample_no):
                ...
                choice_idx = np.random.choice(seq_i_length - memory_size)
                thinned_events = events[choice_idx:(choice_idx + memory_size)]
                thinned_time = times[choice_idx:(choice_idx + memory_size)]
                self.event_cell.append((thinned_events[-1], thinned_events, i))
                self.time_cell.append((thinned_time[-1], thinned_time))
        logger.info('In this dataset, the number of events = {}.'.format(len(self.event_cell)))
        logger.info('Each event is influenced by its last {} historical events.'.format(memory_size))
...
class ThinningSampler(Dataset):
    def __init__(self, database, memorysize:int, sample_no:int):
        ...
        for i in range(len(database['sequences'])):
            ...
            t_start = seq_i['t_start']
            for j in range(sample_no):
                choice_idx = np.random.choice(seq_i_length, memorysize +1 , replace= False)
                thinned_events = events[choice_idx]
                thinned_time = times[choice_idx]
