...
class HawkesProcessIntensity_all(nn.Module):
    ...
    def forward(self, sample_dict):
        ...
        lambda_t = []
        Lambda_T = []
        batch_size = sample_dict['tjs'].shape[0]
        for i in range(sample_dict['tjs'].shape[1] - 1):
        ...
        return torch.cat(lambda_t) / sample_dict['tjs'].shape[1] , torch.cat(Lambda_T)/ sample_dict['tjs'].shape[1]
